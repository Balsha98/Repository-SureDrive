...
const turnCarouselUsingBtns = function () {
    ...
    const activeDot = [...heroDots].find((li) => +$(li).data("hero-car") === currCarIndex);
...
const filterCars = function (event) {
    ...
    checkBoxLists.each((_, ul) => {
        ...
        const isColumn = filterName === "make" || filterName === "fuel" || filterName === "shift";
        ...
        checkedFilters.each((_, box) => {
            ...
            if (isColumn) {
                ...
                for (const limit of ["min", "max"]) data.push(parentListItem.data(limit));
            }
        });
